...
def get_args():
    ...
    parser.add_argument('--drug_name', type=str, required=True,
                        help='Name of the drug(s). For multiple drugs, separate with commas.')
    ...
    parser.add_argument('--savedir', type=str, default='results',
                        help='Directory to save training results and outputs. Default: results')
    ...
    parser.add_argument('--epoch', type=int, default=500, help='Training epoches.')
...
def load_spatial_anndata(args, logs):
    """
    Step 1: Load spatial data into an AnnData object, perform filtering and normalization.
    Returns the AnnData object (Sparse matrix in memory), NOT a dense DataFrame.
    """
    logs.append("Loading spatial data (AnnData stage)...")
    ...
    if args.spatial_count_path.endswith('.h5ad'):
        ...
        # Convert CSV to AnnData, use sparse to save memory
        df_temp = pd.read_csv(args.spatial_count_path, index_col=0)
        spatial_adata = sc.AnnData(
            X=sp.csr_matrix(df_temp.values), # Force sparse matrix storage
    ...
    # --- Part 2: Normalization and Filtering (Scanpy optimized) ---
    if args.perform_normalize:
        logs.append("Performing filtering and normalization (Scanpy optimized)...")
        # Modifies spatial_adata inplace
        sc.pp.filter_genes(spatial_adata, min_cells=5)
        sc.pp.normalize_total(spatial_adata, target_sum=1e6)
        sc.pp.log1p(spatial_adata)
    else:
        logs.append("Skipping normalization.")

    return spatial_adata


def extract_spatial_genes(spatial_adata, bulk_genes, args, logs):
    """
    Step 2: Align Spatial data to Bulk genes and extract.
    Modified to return a Sparse DataFrame if the input is sparse, saving memory.
    """
    logs.append("Aligning Spatial data to Bulk genes and extracting...")
    
    final_df = None
    
    if args.species == 'hs':
        # --- Human Processing ---
        spatial_genes = spatial_adata.var_names.astype(str).str.replace(r"[-_]", ".", regex=True)
        gene_map = dict(zip(spatial_genes, spatial_adata.var_names))
        intersection = sorted(list(set(bulk_genes).intersection(set(spatial_genes))))
        
        if len(intersection) == 0:
            raise ValueError("No intersection found between Bulk and Spatial genes (Human).")
            
        logs.append(f"Intersection found: {len(intersection)} genes.")
        original_genes_to_extract = [gene_map[g] for g in intersection]
        
        # Scanpy slicing (View or Copy)
        subset_adata = spatial_adata[:, original_genes_to_extract]
        
        Xsp = subset_adata.X
        
        if sp.issparse(Xsp):
            # Use pd.DataFrame.sparse.from_spmatrix to maintain sparsity
            # Saves memory compared to Xsp.toarray()
            ...
            # Directly create DataFrame if dense
        ...
        # --- Mouse Processing ---
        logs.append("Processing Mouse -> Human mapping for extraction...")
        
        hsa_gene_map = pd.read_csv('./data/mus_to_hs_mapping.csv')
        # Assume 1st col: Mouse (external_gene_name), 2nd col: Human (homolog)
        hsa_gene_map.rename(columns={
            hsa_gene_map.columns[0]: 'external_gene_name', # Mouse
            hsa_gene_map.columns[1]: 'hsapiens_homolog_associated_gene_name' # Human
        }, inplace=True)
        
        # 1. Filter mapping file: Keep only rows where Human gene is in Bulk
        relevant_mapping = hsa_gene_map[hsa_gene_map['hsapiens_homolog_associated_gene_name'].isin(bulk_genes)]
        
        # 2. Filter mapping file: Keep only rows where Mouse gene is in Spatial Adata
        ...
        if relevant_mapping.empty:
             raise ValueError("No intersection found after Mouse->Human mapping.")

        # 3. Extract the Mouse genes
        mouse_genes_to_extract = relevant_mapping['external_gene_name'].unique()
        logs.append(f"Extracting {len(mouse_genes_to_extract)} Mouse genes that map to Bulk Human genes.")
        ...
        # Build Mouse -> Human dictionary
        mouse_to_human = dict(zip(relevant_mapping['external_gene_name'], relevant_mapping['hsapiens_homolog_associated_gene_name']))
        ...
        # 5. Handle Duplicates (Multiple Mouse genes mapping to same Human gene)
        
        # Check for duplicate column names
        if temp_df.columns.duplicated().any():
            logs.append("Aggregating duplicate Human homologs...")
            # Custom aggregation: revert log -> mean -> log
            def exp_mean_log(x):
                # If Series (no duplicates), return as is
                if isinstance(x, pd.Series): 
                    return x
                # If DataFrame (duplicates), calculate exp-mean-log row-wise
                return np.log(np.expm1(x).mean(axis=1) + 1)

            # Groupby columns and apply function
...
def main():
    ...
    # --- Prepare Common Data --- #
    try:
        spatial_adata_base = load_spatial_anndata(args, logs)
    ...
    preprocess_save_path = './preprocess_results' if args.save_mid else None
    
    if not os.path.exists('./preprocess_results'):
        os.makedirs('./preprocess_results', exist_ok=True)
    ...
    for drug_idx, drug_name in enumerate(drug_list):
        ...
        output_filename = os.path.join(args.savedir, f"{drug_name}_output.txt")
        lines = []
        ...
        if drug_name_norm in smiles_df.iloc[:, 0].values:
            ...
            print(f"\n{'!'*60}")
            print(f"[MISSING DATA] The drug '{drug_name}' was not found in the local SMILES records.")
            print(f"Reference: https://pubchem.ncbi.nlm.nih.gov/ for IsoSMILES")
            print(f"{'!'*60}")
            
            user_input_smiles  = input(f">>> Please enter the Isomeric SMILES for '{drug_name}': ").strip()
            
            while not user_input_smiles:
                print("[ERROR] Input cannot be empty.")
                user_input_smiles = input(f">>> Please enter the Isomeric SMILES for '{drug_name}': ").strip()
                
            drug_smiles = user_input_smiles
        ...
        bulk_genes = X_data_bulk.columns.tolist()
        
        # -------- Step 2. Intersection & VAE -------- #
        try:
            X_data_spatial_curr = extract_spatial_genes(spatial_adata_base, bulk_genes, args, current_logs)
        except Exception as e:
            print(f"Error extracting spatial genes for {drug_name}: {e}")
            continue

        # Now X_data_spatial_curr is a dense DF, aligned (Human)
        # Perform final intersection to ensure consistency
        intersection = sorted(set(bulk_genes).intersection(set(X_data_spatial_curr.columns)))
        current_logs.append(f"[{drug_name}] Final intersection genes: {len(intersection)}")
        ...
        seed_everything(seed)
        vae_file_path = os.path.join('./preprocess_results', f"vae_generated_{drug_name}.parquet")
        ...
        results = train_epoch(
            ...
            graphFunc="GATv2Conv", drugFunc=args.drugFunc, epoches=args.epoch,
            ...
            save_path=args.savedir, drug_name=drug_name, kneigh=args.k_neigh, gene_num=args.gene_num,
            test_size=args.test_size, sampling=args.sampling
        )
        ...
        lines.append(f"l_valid_acc: {results['l_valid_acc']:.4f}")
        lines.append(f"l_valid_f1: {results['l_valid_f1']:.4f}")
        lines.append(f"l_sen_prop: {results['l_sen_prop']:.4f}")
        ...
        pred_path = os.path.join(args.savedir, f"{drug_name}_best.npy")
        if os.path.exists(pred_path):
            pred_arr = np.load(pred_path)
            plot_spatial_sensitivity(loc_category_data_spatial, pred_arr, drug_name, save_dir=args.savedir)
    ...
    if len(drug_list) > 1:
        ...
        for drug in sorted_drugs:
            file_path = os.path.join(args.savedir, f"{drug}_best.npy")
        ...
        if combined_arr is not None and processed_count > 0:
            ...
            save_txt_path = os.path.join(args.savedir, f"{combined_key}_output.txt")
            ...
            plot_spatial_sensitivity(loc_category_data_spatial, combined_arr, combined_key, save_dir=args.savedir)
